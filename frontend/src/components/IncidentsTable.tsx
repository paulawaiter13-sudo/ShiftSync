...
import { formatIncidentCategory } from '../types/incident';
...
const th =
  'whitespace-nowrap px-3 py-2 text-left text-[10px] font-semibold uppercase tracking-wider text-ops-muted sm:px-3.5';
...
export function IncidentsTable({ incidents, onSelect }: IncidentsTableProps) {
  return (
    <DataTable className="ring-1 ring-sev-critical/12">
      <table className="min-w-[48rem] w-full border-collapse text-left text-[13px] leading-snug text-ops-foreground sm:min-w-[56rem]">
        <thead className="sticky top-0 z-10 border-b border-ops-border bg-ops-elevated/95 text-ops-muted shadow-[0_1px_0_0_rgba(0,0,0,0.35)] backdrop-blur-sm">
          <tr>
            ...
            <th className={`${th} hidden lg:table-cell`}>Assigned to</th>
            <th className={`${th} hidden sm:table-cell`}>Created at</th>
        ...
        <tbody>
          {incidents.map((incident, index) => {
            ...
            return (
              ...
              >
                <td className={`${td} font-mono text-[11px] tabular-nums text-ops-muted`}>
                  <span title={incident.id}>#{incident.id.slice(-8)}</span>
                </td>
                <td className={td}>
                ...
                <td className={`${td} text-right`}>
                  <button
                    ...
                    className="rounded-md border border-sev-critical/35 bg-sev-critical/10 px-3 py-1.5 text-2xs font-semibold uppercase tracking-wide text-sev-critical transition hover:bg-sev-critical/20"
                  >
                    View
                  </button>
                </td>
